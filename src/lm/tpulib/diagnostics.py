...
try:
    ...
    logging.error(
        "Failed to import TPU module, make sure you are " "using version 1.3 or above"
    )
...
class Diagnostics(object):
    ...
    def _gather_vm_stats(self):
        ...
        try:
            ...
            logging.error("Failed to get the instance info from the metadata service")
...
def main(argv=None):
    ...
    parser.add_argument(
        "--project_id", default=None, help="ProjectId of the current job"
    )

    parser.add_argument(
        "--tpu_name", required=True, help="Name of the TPU being diagnosed"
    )
