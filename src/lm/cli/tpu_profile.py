...
from tensorflow.python.framework import errors, versions
from tensorflow.python.platform import gfile
from tensorflow.python.platform import tf_logging as logging

from lm import version as profiler_version

FLAGS = flags.FLAGS


def parse_args(name_unused, args_unused):
...
def monitoring_helper(
    service_addr, duration_ms, monitoring_level, display_timestamp, num_queries
):
...
def main(args):
    ...
    if FLAGS.monitoring_level > 0:
        ...
        monitoring_helper(
            service_addr,
            duration_ms,
            FLAGS.monitoring_level,
            FLAGS.display_timestamp,
            FLAGS.num_queries,
        )
    else:
        if not FLAGS.logdir:
